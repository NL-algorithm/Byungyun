...
pr bfs(int sx, int sy, const vector<vector<int>> &board) {
    static const pr dir[3] = {{0, -1}, {-1, 0}, {0, 1}};
...
int trial(vector<vector<int>> &board) {
    set<pr> buffer;
    int ret = 0;
    for (int i = 1; i <= 3; i++) {
        auto can = bfs(n, archers[i], board);
        if (can.first != -1) {
            buffer.insert(can);
        }
    }
